(() => {
  ...
  function update(){
    ...
    const index=Math.max(0,Math.min(3,Math.floor((64-r.top)/Math.max(1,travel)*4)));
    if(index!==active){active=index;scenes.forEach((s,i)=>{s.classList.toggle('is-active',i===active);s.inert=i!==active;s.setAttribute('aria-hidden',String(i!==active));});nav.querySelector('[data-reel-counter]').textContent=`0${active+1} / 04`;previous.disabled=active===0;next.disabled=active===3;}
    playback();
  }
  function configure(){const enabled=!reduced.matches&&tall.matches;story.classList.toggle('is-enhanced',enabled);nav.hidden=!enabled;active=-1;scenes.forEach(s=>{s.inert=false;s.removeAttribute('aria-hidden');s.classList.remove('is-active');s.querySelector('video').pause();});update();}
  function jump(delta){const target=Math.max(0,Math.min(3,active+delta));const travel=story.offsetHeight-story.querySelector('.reel-stage').offsetHeight;window.scrollTo({top:scrollY+story.getBoundingClientRect().top-64+(target+.15)/4*travel,behavior:'smooth'});}
